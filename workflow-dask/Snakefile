...
rule all:
    input:
        expand(data_dir/"{subset_name}-{filter}-region_summary_table.csv",
            subset_name=config["sample_subsets"].keys(),
            filter=config["filters"].keys()
        ),
        expand(data_dir/ "trees" / "{subset_name}-{region_name}-{filter}" / "{subset_name}-{region_name}-{filter}-truncate-{truncation}-mm{mismatch}-post-processed.trees",
            subset_name=config["sample_subsets"].keys(),
...
rule subset_zarr_vcf:
    input:
        lambda wildcards: [(data_dir/"zarr_vcfs"/f"chr{steps.parse_region(config['regions'][wildcards.region_name])[0]}"/"data.zarr"/suffix)
           for suffix in [".vcf_done", "variant_ancestral_allele", "variant_duplicate_position_mask", "variant_bad_ancestral_mask", "variant_no_ancestral_allele_mask"]
        ],
        lambda wildcards: config['sample_subsets'][wildcards.subset_name],
    output:
        data_dir/"zarr_vcfs_subsets"/"{subset_name}-{region_name}-{filter}"/"data.zarr"/".subset_done"
    resources:
        dask_cluster=5,
        mem_mb=16000,
        time_min=4 * 60,
...
rule post_subset_filters:
    input:
        data_dir/"zarr_vcfs_subsets"/"{subset_name}-{region_name}-{filter}"/"data.zarr"/".subset_done"
    output:
        directory(data_dir/"zarr_vcfs_subsets"/"{subset_name}-{region_name}-{filter}"/"data.zarr"/"variant_mask")
    resources:
        dask_cluster=5,
        mem_mb=16000,
        time_min=4 * 60,
...
rule zarr_stats:
    input:
        data_dir/"zarr_vcfs_subsets"/"{subset_name}-{region_name}-{filter}"/"data.zarr"/"variant_mask"
    output:
...
checkpoint summary_table:
    input:
        lambda wildcards: [data_dir/"zarr_stats"/f"{wildcards.subset_name}-{region_name}-{wildcards.filter}"/"stats.json" for region_name in config["regions"].keys()]
    output:
...
def get_ancestor_gen_memory(wildcards):
    ...
    checkpoint_output = checkpoints.summary_table.get(subset_name=wildcards.subset_name, filter=wildcards.filter)
    region_stats = data_dir / "zarr_stats" / f"{wildcards.subset_name}-{wildcards.region_name}-{wildcards.filter}" / "stats.json"
    with open(region_stats, "r") as json_stats_f:
        ...
        mem = 16_000 + int((((n_sites-n_masked) - ac1) * n_samples * n_ploidy) / (8 * 1_048_576))
        return mem
...
rule generate_ancestors:
    input:
        ...
        data_dir/"zarr_stats"/"{subset_name}-{region_name}-{filter}"/"stats.json"
    output:
        data_dir/"ancestors"/"{subset_name}-{region_name}-{filter}"/"ancestors.zarr"
...
rule truncate_ancestors:
    input:
        data_dir/"ancestors"/"{subset_name}-{region_name}-{filter}"/"ancestors.zarr"
    output:
        data_dir/"ancestors"/"{subset_name}-{region_name}-{filter}"/"ancestors-truncate-{lower}-{upper}-{multiplier}.zarr"
    threads: 1
    resources:
        mem_mb=16000,
        time_min=4 * 60,
...
rule match_ancestors:
    input:
        ...
        data_dir/"zarr_vcfs_subsets"/"{subset_name}-{region_name}-{filter}"/"data.zarr"/"variant_mask",
    output:
        data_dir/"ancestors"/"{subset_name}-{region_name}-{filter}"/"ancestors-truncate-{lower}-{upper}-{multiplier}.trees",
        data_dir/"ancestors"/"{subset_name}-{region_name}-{filter}"/"ancestors-truncate-{lower}-{upper}-{multiplier}-performance_report.html"
    threads: config['max_threads']
    resources:
...
rule match_samples:
    ...
    output:
        data_dir/"trees"/"{subset_name}-{region_name}-{filter}"/"{subset_name}-{region_name}-{filter}-truncate-{lower}-{upper}-{multiplier}-mm{mismatch}-raw.trees",
        data_dir/"samples"/"{subset_name}-{region_name}-{filter}-truncate-{lower}-{upper}-{multiplier}-mm{mismatch}"/"performance_report.html"
...
rule post_process:
    input:
        data_dir / "trees" / "{subset_name}-{region_name}-{filter}" / "{subset_name}-{region_name}-{filter}-truncate-{lower}-{upper}-{multiplier}-mm{mismatch}-raw.trees"
    output:
        data_dir / "trees" / "{subset_name}-{region_name}-{filter}" / "{subset_name}-{region_name}-{filter}-truncate-{lower}-{upper}-{multiplier}-mm{mismatch}-post-processed.trees",
